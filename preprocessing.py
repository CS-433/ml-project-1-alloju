...
def angle_values(x, title):
    ...
    """
    columns_id = [
        idx for idx, s in enumerate(title) if "phi" in s and "centrality" not in s
    ]
...
def remove_outliers(x, title):
    ...
    for j in range(len(x[1])):
        ...
        if (
            len(feature_removed) == 0
            or len(feature_removed_0) == 0
            or len(feature_removed_1) == 0
        ):
...
def pca(x, percentage, feature_to_keep):
    ...
    if feature_to_keep == 0:
        explained_percentage = np.squeeze(
            (cumulative_variance_explained > percentage).nonzero()
        )
